...
func (s *Monitor) GetMemoryUsage(ctx gtype.Context, ps gtype.Params) {
	results := make(model.MonitorMemoryPercentCollection, 0)
	...
	for i := 0; i < c; i++ {
		...
		result := &model.MonitorMemoryPercent{
	...
	sort.Sort(results)
	ctx.Success(&model.MonitorMemoryUsage{
...
func (s *Monitor) GetMemoryUsageDoc(doc gtype.Doc, method string, uri gtype.Uri) {
	...
	function.SetOutputDataExample(&model.MonitorMemoryUsage{
		MaxCount: 60,
		CurCount: 1,
		Percents: []*model.MonitorMemoryPercent{
			{
...
func (s *Monitor) sentMemoryUsage(item *NetworkMemoryPercent) {
	...
	argument := &model.MonitorMemoryPercent{
	...
	go s.writeOptMessage(socket.WSMemUsage, argument)
}
